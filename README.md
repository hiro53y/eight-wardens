# Eight Wardens：七人の防衛隊

## 概要
スマホ横向き専用の少人数固定型・育成タワーディフェンスPWAです。React + TypeScript + Vite + Canvas APIで実装し、7人固定の防衛ユニットで左から右へ進む敵を倒して村を守ります。
...
## 2026-05-20 スマホ横向きUI仕上げ
- マップのロック済みステージ表示を黒い四角に見えない鍵風バッジへ変更し、選択ノードとルートの視認性を改善。
- ステージ詳細パネルをスクロール無しで読める密度へ再調整。
- ユニット管理画面のキャラクター画像を枠中央に配置し、一覧・編成プレビューで縦につぶれにくい正方形表示へ変更。
- クラスタリーを画面内に収めるよう、中央詳細パネルの高さ配分と文字密度を調整。
- 戦闘画面の味方配置を上4人/下4人に変更し、敵の進行路を上下から挟んで攻撃する見え方に変更。
